...
X_nn = [ones(m,1),X];
a2 = sigmoid(Theta1*X_nn'); 
a2 = [ones(1,size(a2,2));a2];
h = sigmoid(Theta2*a2)';
J = (1/m)*sum((-y_nn.*log(h)-(1-y_nn).*log(1-h)), 'all') + ...
    (lambda/(2*m))*(sum(Theta1(:,2:end).^2,'all')+sum(Theta2(:,2:end).^2,'all'));
%Part2
delta_3 = h-y_nn;
z2 = Theta1*X_nn';
gGradient_2 = sigmoidGradient(z2)';
delta_2 = delta_3*Theta2(:,2:end).*gGradient_2;
a1 = X_nn;
Delta_1 = zeros(size(Theta1,1),size(Theta1,2));
Delta_2 = zeros(size(Theta2,1),size(Theta2,2));

for i=1:m
    Delta_1 = Delta_1 + delta_2(i,:)'*a1(i,:);
